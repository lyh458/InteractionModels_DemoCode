    ...
    useartdata(false)
{
    ...
    {
        udphost = settingsfile->value("iModMiddleware/OculusTrackingSourceIP").toString();
        udpport = settingsfile->value("iModMiddleware/OculusTrackingSourcePort").toInt();
    }
    else
    {
        qDebug() << "Cant read information from settings file. Using localhost ond port 33777";
        udphost = "127.0.0.1";
        udpport = 33777;
    }
...
void OculusDevice::updateHMDPosition(QVector3D pos)
{
	artposition = pos / 1000;
}
void OculusDevice::updateHMDRotation(QQuaternion quat)
{
	artrotation = quat;
}
...
void OculusDevice::updatePose(unsigned int frameIndex)
{
    ...
    // Fragt die Daten in 6DoF ab: vec3f position und quatf rotation
    ...
    if(useartdata)
    {
        m_position.set(artposition.x(), artposition.y(), artposition.z());
        m_orientation.set(artrotation.x(), artrotation.y(), artrotation.z(), artrotation.scalar());
    }
    else
    {
    ...
    // No matter what -> send the tracking data to the middleware
    QString data = "";
    data += QString::number(pose.Position.x) + " ";
    data += QString::number(pose.Position.y) + " ";
    data += QString::number(pose.Position.z) + " ";
    data += QString::number(pose.Orientation.x) + " ";
    data += QString::number(pose.Orientation.y) + " ";
    data += QString::number(pose.Orientation.z) + " ";
    data += QString::number(-pose.Orientation.w);

    udpsocket->writeDatagram(data.toLocal8Bit(), data.toLocal8Bit().length(), QHostAddress(udphost), udpport);
