...
bool DTrackSDK::receive()
{
	...
	do {
        if (s==NULL){
            std::cout<<"Could not parse ART UDP line. Nothing received"<<std::endl;
            return false;
        }
        
		if (!parseLine(&s)){
			//for(auto i=0;i<len;i++) std::cout<<s[i];
			//std::cout<<std::endl;
            std::cout<<d_udpbuf<<std::endl;
			std::cout<<"Could not parse ART UDP line"<<std::endl;
			return false;
		}
